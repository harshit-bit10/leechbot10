...
async def download_selected_formats(playback_url, audio_formats_selected, video_format, callback_query, download_message, precaption):
    ...
    for audio_format in audio_formats_selected:
        ...
        audio_ydl_opts = {
            ...
            'outtmpl': audio_file_path,  # Ensure audio is saved in the same directory
            'http_headers': ydl_headers,
            'external_downloader': 'aria2c',  # Use aria2c as the external downloader
            'geo_bypass': True,  # Enable geo-bypass
        }

        print(f"Downloading audio format: {audio_format['format_id']}")  # Debugging output ```python
    ...
    await merge_vod_ffmpeg(video_file_path, audio_file_paths, muxed_file_path, download_message, precaption)
...
async def merge_vod_ffmpeg(video_path, audio_paths, output_path, download_message, precaption):
    # Check if video file exists
    ...
    for i in range(len(audio_paths)):
        command.append('-map')
        command.append(f'{i + 1}:a')  # Map each audio stream (1-based index for audio)
    ...
    try:
        await asyncio.to_thread(subprocess.run, command, check=True)  # Use check=True to raise an error if the command fails
